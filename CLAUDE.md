...
\- CYCLE-TIME STRIP (\_cyclePanel) — a SECOND strip below the count quick-filters (FlowLayoutPanel: a static "Cycle time (WIP→Released):" label + a window ComboBox \_cycleWindow [Last 30/90/365 days / All time, default 90] + a result label \_lblCycle), centred by LayoutTopControls. ComputeCycleTime reads the whole audit log (\_all) — INDEPENDENT of the grid's column filters/search (it's a time-window metric, not a view metric): it groups events by file, pairs each Release with the most recent preceding WIP-ENTRY event (IsWipEntry = Create / NewRevision / Unlock / Reinstate), and averages the durations of releases whose RELEASE timestamp falls within the selected window. Shows "{avg} d avg · {median} d median · {N} release(s)" or "no completed cycles in window". A release with no preceding WIP-entry in the log (history began mid-stream) is skipped (unmeasurable). Recomputed in LoadData and on every window-combo change (which also re-centres). Reuses \_summaryFont (no extra disposable); the combo is disposed with the form's control tree.
...
\- Cycle-time analytics: the Audit Report carries a cycle-time strip (\_cyclePanel) — average WIP→Released duration over a selectable window (30/90/365 days / All), computed from audit.csv (ComputeCycleTime) by pairing each Release with the most recent preceding WIP-entry event (Create/NewRevision/Unlock/Reinstate) for the same file and averaging releases within the window. Shows avg + median + count; independent of the grid's filters; recomputes on window change. Hand-checkable per file.
