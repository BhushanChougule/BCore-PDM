...
\- Reason-for-change capture (audit "why"): an interactive Release PROMPTS for a required reason (Cancel aborts the release). "Release only" by design — New Revision does NOT prompt (it just reopens the file for editing), so a Master is asked exactly once per change cycle, and EVERY released version (first release, release-after-revision, release-after-unlock) records a reason. The reason is stored on the RevisionHistory note — the reason LEADS ("{reason}  (REV x)"), since the entry's status line already shows "Released" and the card shows the rev (the old "Released REV x — {reason}" prefix duplicated both and buried the reason behind the ellipsis in the narrow File History panel; the panel note now wraps up to 3 lines) — AND the audit.csv "Release" Note column (the raw reason, searchable in the Audit Report). Threaded through the chained drawing→model release (one reason for the pair); ReleaseFile takes an optional reason parameter so bulk callers can supply a batch reason (they currently pass none → generic note). Closes the PART-2 "audit who/what/when/WHY" gap.
