...
\- UnlockFile(path) → Master only, sets status=WIP, removes OS read-only, then REFRESHES SOLIDWORKS' cached read-only state on the open doc via ReloadOrReplace(readOnly:false) — clearing the disk attribute alone leaves the open document (and any open PARENT ASSEMBLY holding it as a component) showing [Read-only], because SW caches read-only from open time. ReloadOrReplace (the File > Reload API) re-reads the now-writable file and promotes it to read-write EVEN when an assembly holds the component; a plain close+reopen can't (CloseDoc no-ops while the assembly references the doc, handing back the cached read-only doc). ReloadOrReplace returns a swReloadOrReplace STATUS int in this interop (NOT a VARIANT_BOOL — proven by the int-vs-bool compile error): 0 == swReloadOkay (reloaded); any nonzero value is an error. Falls back to close+reopen only when the reload did NOT succeed (reloadRc != 0) or threw
