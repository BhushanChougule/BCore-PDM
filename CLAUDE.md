...
\- Layout: brand title bar ("Advanced Search") · a "SEARCH" main TextBox (cue-banner "Part number, description or file name", same surface as the quick search) · a "REFINE" block with Drawn By (TextBox) + Material (ComboBox) + Finish (ComboBox) + Part Type (ComboBox, Manufactured|Purchased) · a count/status label · a scrollable results area of per-config cards · a Close button. FixedDialog, ClientSize S(440)×S(640). Esc closes (ProcessCmdKey)
...
\- Cards MIRROR the task-pane search cards: rotated status bar (StatusColor green/orange/grey/blue), file name, "PartNo   REV x" (the config's own PN), description (or "→ Superseded by X" for an Obsolete card with a replacement), and Open PRT/ASM + Open DRW buttons. A button sets FileToOpen / FileToOpenConfig (+ OpenDrawing when the drawing must be created) and closes; the task-pane caller (OpenAdvancedSearch) opens it deferred. Open DRW opens an existing drawing directly, else opens the model on the card's config and creates one (OpenDrawing=true → OpenDrawingResult)

\- Returns PARTS/ASSEMBLIES only (the four indexed props live on the model; orphan drawings aren't shown here — the quick search still covers them). GDI hygiene: the results panel rebuilds on every debounce tick via the private ClearAndDispose (Controls.Clear alone parks cards forever — audit C4); fonts + the debounce Timer + the rotated-bar StringFormat are fields disposed in Dispose(bool) (NOT FormClosed — never fires for a form disposed without being shown)
