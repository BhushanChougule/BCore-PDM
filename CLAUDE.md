...
\- CHANGE-IMPACT SUMMARY: below the counts a second line shows the total Qty and total mass deltas — "Qty 13 → 14 (+1)        Weight 8.696 → 8.934 lb (+0.238)". Total Qty = sum of per-line Qty (matches the viewer footer); total Weight = sum of the flattened LEAF weights (sub-assemblies contribute 0 to avoid double-count, matching the viewer footer). Flatten now rolls up LEAF weight per path+config (unit Weight × extended qty on leaf occurrences only — a component with no deeper child), so the per-row Wt Δ deltas sum to the total. AggRow carries Description + Weight; DiffRow carries From/To Description + Weight.

\- CATCH UN-BUMPED CHANGES (the shop's "any change bumps the rev" rule): a Description change is now part of the change test (Changed = rev OR qty OR description differs), and the Description cell shows "old → new" when it changed. A Changed row whose REV letter did NOT move is flagged "Changed *" (cell tooltip "Changed without a revision bump …") and the summary appends "( * N changed w/o rev bump)". (Material is NOT diffed — baselines don't capture it per component; would need a capture-side change.)
