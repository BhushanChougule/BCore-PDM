...
\- Vault Dashboard (Masters only): full-screen, resizable VaultDashboardForm — a sortable/filterable DataGridView of EVERY tracked file (DatabaseManager.GetAllFiles, all statuses, read-only snapshot, no orphan purge). Columns File/PartNo/Description/Status/Rev/Modified By/Modified Date/Released Date/Locked By; Status colour-coded, broken-ref rows flagged red; drawings show their model's PartNo/Description. Content-fit column widths (widest value + 10%); date columns sort chronologically (typed DateTime). Search box + status filter (All/WIP/Released/Locked) filter in memory; summary strip with whole-vault counts. Double-click opens the file (canonical WIP copy via OpenByPath). Export CSV dumps the filtered view. Opened from a Master-only task-pane button.
