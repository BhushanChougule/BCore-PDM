...
\- CONFIG-AWARE SUBJECT (a multi-config part is ONE file but each config is its own Part No): the subject carries \_subjectConfig + \_subjectPartNo so the header, the Find autofill and a picked drawing all reflect the SPECIFIC config — never the file's primary/last-saved config. The header shows "{FileName} · {PartNo}". EnsureSubjectPartNo resolves the PN (caller-supplied → the matching config's PN from the DB record → the record's primary PN → the config name itself, since config == PN). The where-used RESULTS are still computed at the FILE level (GetWhereUsed walks GetDocumentDependencies2, which returns paths with NO per-instance configuration — so config-FILTERED results would require opening every candidate parent; a roadmap follow-up). The task-pane entry seeds \_subjectPartNo from the ACTIVE config (PropertyValidator.GetProperty(doc,"PartNo") + doc.GetActiveConfiguration().Name), and a drawing from GetDrawingPartNo/GetDrawingPrimaryConfig — so switching the active config and clicking Where Used shows that config's PN, not the one active at last save.
