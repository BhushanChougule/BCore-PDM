...
\- DirectChildQty(parentPath, childFileName, cache) → private; how many instances of childFileName the parent DIRECTLY contains, read from the parent's LATEST as-released baseline (its level-0 components, summed by child filename across configs). null = no baseline (never released → unknown, shown as "—"). Cached per parent path for one where-used computation (each parent's baseline read at most once). Released-parents-only by design — WIP assemblies have no baseline yet.
