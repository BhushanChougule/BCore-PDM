...
\- RemoveFromVault(string filePath) → PATH-BASED overload backing the Vault Dashboard row right-click (file need not be open). Runs cheap guards first (Master / tracked / not-Released) so a Released / untracked file never pays the open cost; an ORPHAN (record present, file gone from disk) offers to purge just the dead record (audit "RemoveFromVault" — "orphaned record purged"). Otherwise OpenByPath opens the file transiently — the per-config DrawingNo (needed to scrap PDFs) lives only on the open doc — and delegates to RemoveFromVault(doc), which already closes the doc before moving anything.
