...
\- Export CSV (RFC-4180 + Excel formula-injection guard) dumps the SELECTED baseline. "Export All Revs" writes EVERY captured baseline to one .xlsx via XlsxWriter — a worksheet per release (CSV can't hold multiple sheets), each with a meta block + the full indented BOM. Esc closes; fonts disposed on FormClosed. Docking resolves by z-order so the Fill grid is added FIRST then the edge panels (house convention, cf. VaultDashboardForm).
