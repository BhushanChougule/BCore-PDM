...
DPI-aware read-only Form (S(v)=v\*\_scale, own palette/CSV escaping per the one-form-one-file convention). Opened from the Vault Dashboard row right-click ("Where Used…", models only) — nested-modal ON TOP of the dashboard. Lists the parent assemblies in a read-only DataGridView: Assembly / Part No / Rev / Status (Status colour-coded green/maroon/orange/grey, Obsolete grey). TWO MODES via a radio toggle: DIRECT (default, 1 level) = VaultManager.GetWhereUsed(path), the assemblies that directly reference the file; ALL LEVELS = VaultManager.GetWhereUsedTree(path), the full usage chain up to the top-level assemblies, rendered as an INDENTED TREE (Assembly name indented by Level with a "↳" connector; deeper-level names muted) so a part used via a sub-assembly shows the whole path. The all-levels walk is computed LAZILY on first switch (one disk pass, wait-cursor) and CACHED; the default Direct walk runs in the ctor. Count line: "Used by N assembly(ies)." (direct) or "Used by N reference(s) across the usage chain · D direct." (all levels); empty → "Not used by any tracked assembly." Export CSV (RFC-4180 + formula-injection guard) dumps the ACTIVE mode with a Level column + a Mode header cell. The count label is AutoEllipsis with its width BOUNDED to stop before the buttons (LayoutBottom, re-run on resize) so its text never overlaps Export/Close at any width/DPI. Esc closes; fonts disposed on FormClosed; Fill-grid-first docking (house z-order convention).
