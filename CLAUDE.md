...
\- Data from DatabaseManager.GetAllFiles() (read-only snapshot, all statuses, no orphan purge). Fetched once into \_all; the global search box (300ms debounce timer, matches PartNo/Description/FileName) + the per-column Excel-style filters all filter \_all IN MEMORY via ApplyFilter (no re-query). Summary strip shows Total/WIP/Released/Locked/Broken-refs counts (whole vault) + "(Showing from–to of N · Page X of Y)".
