...
\- A REASON-FOR-CHANGE prompt (required, interactive only) follows the confirm/picker and precedes any archive/bump; stored on the WIP history note + the audit "NewRevision" note. Cancel aborts.
...
\- Reason-for-change capture (audit "why"): an interactive Release and New Revision PROMPT for a required reason (Cancel aborts the operation). The reason is stored on the RevisionHistory note (shows in the task-pane File History) AND the audit.csv "Release"/"NewRevision" Note column (searchable in the Audit Report). Threaded through the chained drawing→model release (one reason for the pair) and accepted as an optional parameter by ReleaseFile/StartNewRevision (and their *Core split) so bulk callers can supply a batch reason (they currently pass none → generic note). Closes the PART-2 "audit who/what/when/WHY" gap for the two formal lifecycle transitions.
