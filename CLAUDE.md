...
\- Filter-popup Select All / Clear beyond the render cap (audit C5): ColumnFilterDialog's Select All / Clear used to iterate \_visibleRaw — the RENDERED list, capped at DisplayCap (2000) — so on a high-cardinality column (File Name at vault scale, exactly what the cap exists for) "Clear, then tick one" flipped only the first 2000 entries, left the rest checked, and committed a filter that quietly allowed almost everything. Now SetMatched flips every value MATCHED by the in-popup search via the shared MatchesTerm predicate (also used by RebuildList so the two loops can't drift). Fixed identically in BOTH private copies (VaultDashboardForm + AuditReportForm, one-form-one-file convention).
