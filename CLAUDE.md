...
\- GetBaselineChildUsage(childFilePath, targetConfig) → the WHERE-USED CONFIG FILTER source (so where-used works by PART NUMBER, not just file, with NO file opening). Reads <Baselines> in ONE load and, for every assembly's LATEST baseline (max ReleasedDate per AssemblyPath), classifies its use of the child file: BaselineChildUsage.ParentsUsingTarget = the baseline has the child under targetConfig; ParentsWithDifferentConfig = it has the child under a KNOWN (non-empty) config that is NOT the target. A parent in NEITHER set has no baseline, or a baseline that captured no config — so the caller keeps it (unverified). MATCHES ON THE COMPONENT CONFIG ONLY, never PartNo: the baseline component's PartNo is enriched from the child's PRIMARY File record (identical for every config), so it can't distinguish configs; the referenced Config can. Keys are normalised FULL paths. Backs VaultManager.BuildConfigUsage.
...
\- GetWhereUsed(filePath, targetPartNo=null) → PUBLIC, on-demand "where used" (SINGLE LEVEL): returns the tracked assemblies that DIRECTLY reference the file (traverse=FALSE → top-level components only, so the DIRECT-parent set), each as a WhereUsedEntry (Path/Name + PartNo/Revision/Status from the parent's vault record, Level=0, Qty = direct instance count from the parent's latest baseline). Same disk-walk primitive + best-effort caveat as GetParentAssemblies; no UI/open needed. CONFIG FILTER: when targetPartNo (= a configuration name) is given, each parent is filtered via BuildConfigUsage → DatabaseManager.GetBaselineChildUsage (the as-released baselines, ONE vault.xml read, no file opening): a parent is DROPPED only when its latest baseline proves the child is used under a DIFFERENT known config, and KEPT when it uses the target config OR has no baseline / no captured config (unverified). null ⇒ file-level (every config), as before. Backs WhereUsedForm's default "Single Level" mode. WhereUsedEntry is a top-level class in VaultManager.cs (like BatchResult); its Level field is the depth in a multi-level walk (0 = direct parent), Qty is a nullable per-edge instance count (null = the parent has no baseline → "—").
...
\- BuildConfigUsage(filePath, targetConfig) + ParentPassesConfigFilter(parentPath, usage) → private; the config-aware where-used filter shared by all three modes. BuildConfigUsage returns null (no filtering) when targetConfig is empty, else DatabaseManager.GetBaselineChildUsage (try/catch → null, so any DB failure degrades to file-level, never crashes). ParentPassesConfigFilter keeps a parent in usage.ParentsUsingTarget, drops one in usage.ParentsWithDifferentConfig, and keeps everything else (unverified).
...
\- CONFIG-AWARE SUBJECT + RESULTS (a multi-config part is ONE file but each config is its own Part No): the subject carries \_subjectConfig + \_subjectPartNo + \_subjectMultiConfig so the header, the Find autofill and a picked drawing all reflect the SPECIFIC config — never the file's primary/last-saved config. The header shows "{FileName} · {PartNo}". EnsureSubjectPartNo (run ONCE per subject, \_subjectResolved guard) reads the DB record to resolve the PN (caller-supplied → matching config's PN → record's primary PN → config name, since config == PN) AND whether the file is multi-config. CONFIG-FILTERED RESULTS via the as-released BASELINES (the user's "use the stored assembly data" idea — no file opening): for a MULTI-CONFIG subject, ConfigTarget() returns the config name and all three GetWhereUsed\* calls pass it; VaultManager filters each DIRECT parent against DatabaseManager.GetBaselineChildUsage — a parent is DROPPED only when its latest baseline PROVES the child is used under a different (known) config, and KEPT when it uses the target config OR has no baseline / no captured config (unverified, shown with Qty "—"). Matched on the baseline component CONFIG, not PartNo (the baseline's PartNo is enriched from the child's PRIMARY record and is identical across configs). SINGLE-config subjects and the dashboard's file-level right-click pass null ⇒ no filtering (file-level, every config — avoids a stale-baseline false-exclude on "Default"-config parts). The task-pane entry seeds \_subjectPartNo from the ACTIVE config (PropertyValidator.GetProperty(doc,"PartNo") + doc.GetActiveConfiguration().Name), and a drawing from GetDrawingPartNo/GetDrawingPrimaryConfig — so switching the active config and clicking Where Used shows (and filters by) that config, not the one active at last save. LIMITATION: the baseline reflects the assembly's LAST RELEASE, so a WIP assembly that swapped a child's config since its last release can be stale (kept as unverified if its baseline lacks the config, or mis-classified if it recorded the old one); released assemblies are accurate (frozen).
