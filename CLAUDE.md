...
\- DatabaseManager.GetBaselines(asmPath) loads all captured baselines (most recent first). A ComboBox "Release:" selector (widened, DropDownWidth set so the date never clips) switches between revisions; meta line shows PartNo · REV · ReleasedBy/Date. Dates DISPLAY as MM/dd/yyyy HH:mm:ss via FmtDate (storage stays ISO "yyyy-MM-dd HH:mm:ss" so it sorts chronologically). A read-only DataGridView lists Component / Part No / Rev / Status / Qty — rendered as an INDENTED TREE: rows stay in stored depth-first order (NOT re-sorted), the Component name is indented by its Level, and sub-assembly rows get a ▸ marker + bold so their nested parts read underneath them. Status colour-coded green/maroon/orange. Columns are NotSortable (sorting would break the tree). Empty list → "No baselines captured yet". Export CSV adds a Level column + indented name so the structure survives in Excel.
