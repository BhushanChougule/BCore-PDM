...
\- DatabaseManager.GetBaselines(asmPath) loads all captured baselines (most recent first). A ComboBox "Release:" selector (widened, DropDownWidth set so the date never clips) switches between revisions; meta line shows PartNo · REV · ReleasedBy/Date. Dates DISPLAY as MM/dd/yyyy HH:mm:ss via FmtDate (storage stays ISO "yyyy-MM-dd HH:mm:ss" so it sorts chronologically). A read-only DataGridView lists Component / Part No / Rev / Status / Qty — rendered as an EXPANDABLE INDENTED TREE: rows stay in stored depth-first order (NOT re-sorted), the Component name is indented by its Level, and sub-assembly rows show ▾ (expanded) / ▸ (collapsed) + bold. The tree is driven manually (DataGridView isn't native-tree): RenderRows walks _full honouring a _collapsed set (a collapsed node hides every deeper row until the level returns to its own — works for nested sub-sub-assemblies), and _rowToComp maps grid rows back to component indices. Clicking a sub-assembly's Component cell (Grid_CellClick, deferred via BeginInvoke to avoid re-entrancy) toggles it; "Expand All" / "Collapse All" buttons give the full tree or top-level-only view. _collapsed resets on each release switch (defaults fully expanded). Status colour-coded green/maroon/orange; columns NotSortable. Empty list → "No baselines captured yet". Export CSV ALWAYS dumps the FULL tree (all levels, ignoring collapse state) with a Level column + indented name so the structure survives in Excel.
