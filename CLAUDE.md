...
DPI-aware read-only Form (S(v)=v\*\_scale, own palette/CSV escaping per the one-form-one-file convention). Opened from the Vault Dashboard row right-click ("Where Used…", models only) — nested-modal ON TOP of the dashboard. The top block (filename, subtitle, the mode radios and a "double-click to open" tip) is CENTRED horizontally via LayoutTop (re-run on top-panel resize), matching the other BCore forms. Lists the parent assemblies in a read-only DataGridView: Assembly / Part No / Rev / Status (Status colour-coded green/maroon/orange/grey, Obsolete grey). TWO MODES via a radio toggle: "Single Level" (default) = VaultManager.GetWhereUsed(path), the assemblies that directly reference the file; "All Levels (Incl. Sub-assemblies)" = VaultManager.GetWhereUsedTree(path), the full usage chain up to the top-level assemblies, rendered as an INDENTED TREE (Assembly name indented by Level with a "↳" connector; deeper-level names muted) so a part used via a sub-assembly shows the whole path. The all-levels walk is computed LAZILY on first switch (one disk pass, wait-cursor) and CACHED; the default single-level walk runs in the ctor. Count line: "Used by N assembly." / "Used by N assemblies." (single level) or "Used by N reference(s) · D direct." (all levels — rows are occurrences, so the same assembly can appear under several children); empty → "Not used by any tracked assembly." DOUBLE-CLICK a row → sets FileToOpen + closes; the dashboard caller (MenuWhereUsed) then OpenDeferred()s it (canonical WIP copy via OpenByPath), mirroring the dashboard's own double-click and BaselineViewerForm. Export CSV (RFC-4180 + formula-injection guard) dumps the ACTIVE mode with a Level column + a Mode header cell. The count label is AutoEllipsis with its width BOUNDED to stop before the buttons (LayoutBottom, re-run on resize AND after every LoadRows) so its text never overlaps/clips under Export/Close at any width/DPI. Esc closes; fonts disposed on FormClosed; Fill-grid-first docking (house z-order convention).
...
\- Where Used (read-only): right-click a part/sub-assembly in the Vault Dashboard → "Where Used…" lists the parent assemblies, each with its Part No / Rev / Status, in TWO selectable modes — DIRECT (default, 1 level; VaultManager.GetWhereUsed) shows the assemblies that directly contain it; ALL LEVELS (VaultManager.GetWhereUsedTree) digs deeper to show the full usage chain — sub-assemblies and the bigger assemblies that contain them — as an indented tree. Always current (on-demand dependency-walk the release gate uses, no persisted index); the multi-level walk builds the reverse-dependency map in one disk pass then walks in memory. Double-click a parent row to open that assembly (deferred through the dashboard like its own double-click). Export CSV (with a Level column) for impact analysis. (A persisted where-used index for scale is a roadmap follow-up.)
