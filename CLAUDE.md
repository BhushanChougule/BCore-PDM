...
\- Column widths are CONTENT-FIT: AutoSizeColumnsMode=None, then AutoSizeColumns() sets each column to GetPreferredWidth(AllCells)×1.20 (widest header/cell + 20%), clamped S(48)..S(520), re-run on every filter — so columns stay tidy/uniform instead of evenly stretched.
...
\- Control row (search box, status filter, Refresh, Export CSV) is height-matched: all set to the ComboBox's font-derived PreferredHeight so the row lines up. A SECOND filter row below it holds two labelled engineer dropdowns — "Modified By:" and "Released By:" (each "All" + the sorted distinct usernames from the data) — so a Master can isolate one engineer's saves and/or releases. The summary strip sits below with the panel tightened to it (no dead space). The header label reads "BCore VAULT DASHBOARD". The title, BOTH control rows and the summary are CENTER-aligned horizontally via LayoutTopControls (called on load + every top-panel resize).
...
\- Data from DatabaseManager.GetAllFiles() (read-only snapshot, all statuses, no orphan purge). Fetched once into \_all; a search box (300ms debounce timer, matches PartNo/Description/FileName) + a status filter ComboBox (All/WIP/Released/Locked) + the Modified By / Released By engineer dropdowns all filter \_all IN MEMORY via ApplyFilter (no re-query; engineer match is case-insensitive Eq). The engineer dropdowns are rebuilt from the data on every LoadData by PopulateEngineerFilters (preserving the current pick if that user is still present); a \_populating guard stops their SelectedIndexChanged from filtering mid-rebuild. Summary strip shows Total/WIP/Released/Locked/Broken-refs counts (whole vault) + "(showing N)".
