...
\- CaptureAssemblyBaseline(doc, asmPath, partNo, rev, config, user) → ResolveTree(doc) (fallback ResolveComponents on empty) then DatabaseManager.SaveAssemblyBaseline; audit-logs "BaselineCaptured" (or "BaselineFailed"). NON-FATAL (try/catch swallow, same contract as ExportBom). Called from ReleaseFile right AFTER the assembly's status flips to Released. POCOs AssemblyBaseline (AssemblyPath/Name, PartNo, Revision, Config, ReleasedBy/Date, Components) + BaselineComponent (Path, Name, PartNo, Revision, Status, Qty, Level) live here and are shared with DatabaseManager (same namespace/assembly). Components persist with a Level attribute (absent → 0 on pre-indent baselines, which then render flat).
...
\- DatabaseManager.GetBaselines(asmPath) loads all captured baselines (most recent first). A ComboBox "Release:" selector (widened, DropDownWidth set so the date never clips) switches between revisions; meta line shows PartNo · REV · ReleasedBy/Date. Dates DISPLAY as MM/dd/yyyy HH:mm:ss via FmtDate (storage stays ISO "yyyy-MM-dd HH:mm:ss" so it sorts chronologically). A read-only DataGridView with columns No. / Component / Part No / Rev / Status / Qty — rendered as an EXPANDABLE INDENTED TREE. The FIRST column "No." is the tree column: indent (by Level) + a ▾ (expanded) / ▸ (collapsed) arrow on sub-assemblies + the BOM OUTLINE NUMBER (1, 2 … at the top; N.1, N.2 … under N; N.M.1 … one level deeper) computed once per baseline in LoadSelected (intrinsic to tree position, so unchanged by collapse). Component is the plain name. All column HEADERS are centre-aligned (ColumnHeadersDefaultCellStyle.Alignment); Rev + Qty VALUES centre-aligned, the rest left. Rows stay in stored depth-first order (NOT re-sorted). The tree is driven manually (DataGridView isn't native-tree): RenderRows walks _full honouring a _collapsed set (a collapsed node hides every deeper row until the level returns to its own — works for nested sub-sub-assemblies), and _rowToComp maps grid rows back to component indices. Clicking a sub-assembly's No. or Component cell (Grid_CellClick, deferred via BeginInvoke to avoid re-entrancy) toggles it; sub-assembly No./Component cells are bold. "Expand All" / "Collapse All" buttons give the full tree or top-level-only view. _collapsed resets on each release switch (defaults fully expanded). Status colour-coded green/maroon/orange; columns NotSortable. Empty list → "No baselines captured yet". Export CSV ALWAYS dumps the FULL tree (all levels, ignoring collapse state) with No. (outline) + Level columns + indented name so the structure survives in Excel.
