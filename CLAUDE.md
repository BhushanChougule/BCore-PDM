...
\- Layout: brand title bar ("Advanced Search") · a CENTRED "SEARCH" main TextBox (cue-banner "Part number, description or file name", same surface as the quick search) · a "REFINE" block as a 2×2 grid (Drawn By TextBox + Material ComboBox on row 1; Finish ComboBox + Part Type ComboBox on row 2) · a centred count/status label · a scrollable results area of per-config cards · a Close button. Section headers + hint + count are centre-aligned. FixedDialog, ClientSize S(648)×S(652) — height = header + filters + bottom + a results area sized to EXACTLY 5 card rows × 3 cards (15) so no 6th row peeks (viewport ~404 vs 400 of content)

\- Dropdowns are built from PropertyForm.MaterialOptions()/FinishTypeOptions()/PartTypeOptions() (single source of truth) with a leading "— Any —" sentinel (index 0 = no filter) and the property dialog's "-- Select --" stripped. Material + Finish are TYPE-TO-NARROW combos (DropDownStyle=DropDown + AutoCompleteMode.Suggest/AutoCompleteSource.ListItems — typing filters the dropdown suggestions; the material list will grow long); Part Type is a plain DropDownList (two values). FlatStyle left at the default Standard (the old FlatStyle.Flat rendered a dot-sized arrow). ComboValue reads the combo's TEXT (so an editable pick is honoured), returns "" for "— Any —"/empty, and for an editable combo treats a mid-type PARTIAL that doesn't yet match a real list value as "" (no filter) so typing only narrows the list — the match stays EXACT on a committed value
