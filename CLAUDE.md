...
\- ExportFlatPatternOnly(doc, exportRoot, stamp) → exports flat-pattern DXF only; called once for the original active config after the multi-config STEP loop. Sheet-metal detection scans GetFeatures(FALSE) — the whole feature list INCLUDING sub-features — because multibody sheet metal nests each body's SheetMetal feature inside its body folder and the old top-level-only scan silently skipped those parts
