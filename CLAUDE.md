...
\- ResolveComponents(asmPath) → walks the assembly's FULL multi-level dependency tree from disk via GetDocumentDependencies2(asmPath, true, true, false) (traverse=true → every descendant; same primitive GetUnreleasedComponentsByPath/GetParentAssemblies use — no UI open needed). Returns each UNIQUE component file (part or sub-assembly) with an occurrence-count Qty. Toolbox/standard hardware skipped; drawings excluded (not components); the assembly itself skipped. PartNo/Revision/Status are left blank here — DatabaseManager fills them from the live File records in ONE load when persisting (so the vault lock is never re-acquired per component).

\- CaptureAssemblyBaseline(asmPath, partNo, rev, config, user) → ResolveComponents then DatabaseManager.SaveAssemblyBaseline; audit-logs "BaselineCaptured" (or "BaselineFailed"). NON-FATAL (try/catch swallow, same contract as ExportBom). Called from ReleaseFile right AFTER the assembly's status flips to Released — the release gate has already guaranteed every tracked child is Released, so each child's record carries its released rev. POCOs AssemblyBaseline (AssemblyPath/Name, PartNo, Revision, Config, ReleasedBy/Date, Components) + BaselineComponent (Path, Name, PartNo, Revision, Status, Qty) live here and are shared with DatabaseManager (same namespace/assembly).

\- LIMITATIONS (documented, future work): Qty is an occurrence count from the dependency array, not a true per-instance BOM quantity (real Qty needs the live component tree — see ExportBom); for a MULTI-CONFIG child the snapshot records its DB primary Revision and the baseline records the assembly's ACTIVE config at release time. Phase-2 "indented BOM" (FEATURE_ROADMAP.md) promotes this to a true multi-level BOM.
