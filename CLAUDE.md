...
2\. Search (auto-search 600ms timer, ≥2 chars, Enter key via ProcessCmdKey). Button layout (audit M12): Clear (S22 at w−S73) and Search (S48 at w−S48) with a real S(3) gap — the old positions overlapped by S(18) with Clear ON TOP, so clicks on the left 40% of "Search" silently cleared the box; the search box width leaves S(78) for both buttons
