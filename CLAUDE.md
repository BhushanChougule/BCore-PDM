...
\- GetComponentQtys(asmPath) → direct-child instance counts (filename → summed Qty) from an assembly's CURRENT `<Components>` snapshot, or null when it has none (caller falls back to the baseline). Lets Where Used show a real Qty for WIP assemblies too (VaultManager.DirectChildQty prefers it over the baseline).
...
\- DirectChildQty(parentPath, childFileName, cache) → private; how many instances of childFileName the parent DIRECTLY contains. PREFERS the parent's CURRENT `<Components>` snapshot (DatabaseManager.GetComponentQtys — captured at every save, so a WIP assembly has a real Qty too), FALLING BACK to its LATEST as-released baseline (level-0 components, summed by child filename across configs) when it has no snapshot yet. null = neither exists (shown as "—"). Cached per parent path for one where-used computation (each parent read at most once).
