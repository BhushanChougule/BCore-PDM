...
\- SearchFilesAdvanced(mainTerm, drawnBy, material, finish, partType, out truncated) → the ADVANCED (property-wide) search backing AdvancedSearchForm. AND-combines whatever fields are filled and matches them PER CONFIGURATION: mainTerm = substring over file name + file-level PN/Description + the config's PN/Description; drawnBy = substring over the config's DrawnBy; material/finish/partType = EXACT (case-insensitive) match on the config's Material1/FinishType/PartType. At least one field must be non-empty (else empty). Returns PARTS/ASSEMBLIES only (the four indexed props live on the model, not on drawings — the result card's Open DRW still reaches the drawing via the DrawingIndex), each with its Configurations list TRIMMED to only the configs that passed every active filter, so the popup renders exactly the matching config cards (never "all configs of a file that matched") with a real per-config Part No. Capped at MaxSearchResults; READ-ONLY (orphans skipped when the share is reachable but NEVER purged — no write, so it's safe in degraded-lock mode; the quick SearchFiles owns orphan cleanup). The four matched fields are indexed per config in the <Config> block at save time (ConfigEntry gained Material/FinishType/DrawnBy/PartType; captured in OnFileSavePost, round-tripped by BuildConfigsElement/ReadConfigs); legacy records carry empties until re-saved (graceful, no migration)
...
\- Layout: brand title bar ("Advanced Search") · a CENTRED "SEARCH" main TextBox (cue-banner "Part number, description or file name", same surface as the quick search) · a "REFINE" block as a 2×2 grid (Drawn By TextBox + Material ComboBox on row 1; Finish ComboBox + Part Type ComboBox on row 2) · a centred count/status label · a scrollable results area of per-config cards · a bottom bar (Export CSV on the left, Close on the right). Section headers + hint + count are centre-aligned. FixedDialog, ClientSize S(648)×S(652) — height = header + filters + bottom + a results area sized to EXACTLY 5 card rows × 3 cards (15) so no 6th row peeks (viewport ~404 vs 400 of content)

\- Dropdowns are built from PropertyForm.MaterialOptions()/FinishTypeOptions()/PartTypeOptions() (single source of truth), the property dialog's "-- Select --" stripped. Material + Finish are TYPE-TO-NARROW combos (DropDownStyle=DropDown + AutoCompleteMode.Suggest/AutoCompleteSource.ListItems — typing filters the dropdown suggestions; the material list will grow long) and start BLANK with a grey cue-banner placeholder ("Type or pick a material/finish" via CB_SETCUEBANNER, the combo analogue of EM_SETCUEBANNER) — no "— Any —" item, empty = no filter, just like the search box. Part Type is a plain DropDownList (two values) and keeps a leading "— Any —" sentinel (a DropDownList has no edit box, so no cue banner). FlatStyle left at the default Standard (the old FlatStyle.Flat rendered a dot-sized arrow). ComboValue reads the combo's TEXT (so an editable pick is honoured), returns "" for ""/"— Any —", and for an editable combo treats a mid-type PARTIAL that doesn't yet match a real list value as "" (no filter) so typing only narrows the list — the match stays EXACT on a committed value

\- LIVE search: a 450ms debounce Timer re-runs on every field change (main + drawnBy TextChanged, the three combos' SelectedIndexChanged). RunSearch reads the fields, returns early with a hint when ALL are empty, else calls DatabaseManager.SearchFilesAdvanced (try/catch → "Vault unavailable"). Cards are built ONE per config in each returned file's already-TRIMMED Configurations (so they're exactly the matching configs), the Open DRW wiring resolved against ONE DatabaseManager.BuildDrawingIndex snapshot (not per card). Capped at MaxCards=50 with a "refine to narrow" hint

\- Cards MIRROR the task-pane search cards (rotated status bar via StatusColor green/orange/grey/blue, file name, "PartNo   REV x" = the config's own PN, description or "→ Superseded by X" for an Obsolete card, Open PRT/ASM + Open DRW) but are laid out in a 3-COLUMN GRID (RenderCards): each ~task-pane-width card is a white tile with a 1px border (shared _penBorder, drawn in card.Paint — no per-paint alloc) separated by gaps, tiled left-to-right/top-to-bottom; card width derives from the column width. A button sets FileToOpen / FileToOpenConfig (+ OpenDrawing when the drawing must be created) and closes; the task-pane caller (OpenAdvancedSearch) opens it deferred. Open DRW opens an existing drawing directly, else opens the model on the card's config and creates one (OpenDrawing=true → OpenDrawingResult)

\- EXPORT CSV (bottom-left button, disabled until a search returns rows): writes the SHOWN result rows (one per config card, ≤ MaxCards) with columns File Name, Part No, Drawing No, Description, Rev, Material, Finish, Drawn By, Part Type — per-config fields read from the matched ConfigEntry stored on each card; PartNo/Description/Rev use the card's resolved values (config value, file-level fallback). RFC-4180 + Excel formula-injection guard (the house Csv helper). SaveFileDialog with a timestamped default name
...
\- Advanced (property-wide) search: a popup (AdvancedSearchForm) reached by DOUBLE-CLICKING the task-pane "SEARCH FILES" header — the quick box stays a clean PartNo/Description/FileName quick-find, so property search never floods a part-name lookup (typing "ste" no longer drags in every STEEL part). A main box (same surface as the quick search) + Drawn By / Material / Finish / Part Type filters, AND-combined and matched PER CONFIGURATION (DatabaseManager.SearchFilesAdvanced): main + drawnBy = substring, material/finish/partType = exact (dropdowns sourced from PropertyForm's option lists, "— Any —" = no filter). Results are the SAME per-config cards as the quick search (status bar, PN+REV, description, Open PRT/ASM + Open DRW), one per MATCHING config (a multi-config part shows only its STEEL config, with that config's own Part No — not "all configs of a matching file"). The four fields are indexed per config in the <Config> block at save (ConfigEntry gained Material/FinishType/DrawnBy/PartType, captured in OnFileSavePost, round-tripped by BuildConfigsElement/ReadConfigs); legacy records carry empties until re-saved (graceful, no migration). Live debounced search; capped at 50 cards + refine hint; READ-ONLY (no orphan purge). Material/Finish are blank type-to-narrow combos (cue banner, no "— Any —"); Part Type is a "— Any —" dropdown. EXPORT CSV button dumps the shown rows (File Name, Part No, Drawing No, Description, Rev, Material, Finish, Drawn By, Part Type). Closes the PART-2 "search on Material/DrawnBy/PartType" gap. (Surfacing these as dedicated Vault Dashboard columns/filters is a roadmap follow-up — the data is now stored for it.)
