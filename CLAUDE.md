...
\- A REASON-FOR-CHANGE prompt (required, interactive only) follows the confirm/picker and precedes any archive/bump; the reason is stored on the WIP history note + the audit "NewRevision" note. Cancel aborts.
...
\- Reason-for-change capture (audit "why"): an interactive Release and New Revision now PROMPT for a required reason (Cancel aborts the operation). The reason is stored on the RevisionHistory note (so it shows in the task-pane File History) AND the audit.csv "Release"/"NewRevision" row's Note column (so it's searchable in the Audit Report). Threaded through the chained drawing→model release (one reason for the pair) and accepted as a parameter by ReleaseFile/StartNewRevision so bulk callers can supply a batch reason (they currently pass none → generic note). Closes the PART-2 "audit who/what/when/WHY" gap for the two formal lifecycle transitions.
