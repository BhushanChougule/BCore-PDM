...
\- Where Used (read-only): right-click a part/sub-assembly in the Vault Dashboard → "Where Used…" lists the parent assemblies, each with Part No / Rev / Qty / Status, in THREE selectable modes — SINGLE LEVEL (default; VaultManager.GetWhereUsed) the assemblies that directly contain it; ALL LEVELS (GetWhereUsedTree) the full usage chain (sub-assemblies and the bigger assemblies that contain them) as an indented tree; TOP LEVEL ONLY (GetWhereUsedTopLevel) just the root/final assemblies. QTY = instances per parent, read from the parent's latest as-released baseline ("—" when never released). A live FILTER box narrows the list; a RIGHT-CLICK menu opens the assembly, climbs further up ("Where Used from here"), copies the path or opens the folder; double-click (or "Open Assembly") opens the parent (deferred through the dashboard like its own double-click). Always current (on-demand dependency-walk the release gate uses, no persisted index); the multi-level/top-level walks build the reverse-dependency map in one disk pass then walk in memory. Export CSV (with Level + Qty columns) for impact analysis. (A persisted where-used index for scale is a roadmap follow-up.)
