...
namespace PDMLite
{
    public class TaskPaneControl : UserControl
    {
        ...
        private void BuildUI()
        {
            ...
            // ── Pending Requests button (Master only) ─────────────────
            this.Controls.Add(Divider(x, y, w));
            y += S(10);
        ...
        // ── Refresh request count on button (Master only) ────────────
        private void RefreshRequests()
        {
            ...
            if (requests.Count > 0)
            {
                _btnRequests.Text = $"PENDING REQUESTS  ({requests.Count})";
                _btnRequests.BackColor = cOrange;
            }
            else
            {
                _btnRequests.Text = "PENDING REQUESTS";
                _btnRequests.BackColor = cBrandDark;
            }
        }

        private void OpenRequestsPopup()
        {
            var popup = new PendingRequestsForm(_scale);
